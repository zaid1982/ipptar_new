...
/* 	$select = "
	...
	$row = sqlSelect(
		'SELECT * AS total FROM epenilaian WHERE e_pid LIKE ? ORDER BY e_id ASC', 
		array($pid, $kid)
	);
	$rowCount = sqlSelect(
		'SELECT count(e_pid) AS total FROM epenilaian WHERE e_pid LIKE ? ORDER BY e_id ASC', 
		array($_POST['pid'], $kid)
	);
	...
	if($rowCount['total'] > 0) {
		...
		// add parameterized query
		$result = sqlUpdate(
			'UPDATE epenilaian SET e_j1 = ?, e_j2 = ?, e_j3 = ?, e_j4 = ?, e_j5 = ?, e_j6 = ?, e_j7 = ?, e_j8 = ?, e_j9 = ?, e_j10 = ?, e_j11 = ?, e_j12 = ?, e_j13 = ?, e_j14 = ?, e_j15 = ?, e_j16 = ?, e_j20 = ?, e_j21 = ?, e_j22 = ?, e_date = ? WHERE e_pid = ?',
			array(
				$_POST['j1'], 
				$_POST['j2'], 
				$_POST['j3'], 
		...
		// add parameterized query
		$row = sqlSelect(
			'SELECT * AS total FROM epenilaian_ticer WHERE e_pid LIKE ? ORDER BY e_id ASC', 
			array($pid, $kid)
		);
		$rowCount = sqlSelect(
			'SELECT count(e_pid) AS total FROM epenilaian_ticer WHERE e_pid LIKE ? ORDER BY e_id ASC', 
			array($_POST['pid'], $kid)
		);
		...
		if($rowCount['total'] > "0") {
			...
			for($i=0; $i<$item_idCount; $i++) {
				...
				// add parameterized query
				$result = sqlUpdate(
					'UPDATE epenilaian_ticer SET e_j17 = ?, e_j18 = ?, e_j19 = ? WHERE e_pid = ? AND e_tid = ?', 
					array(
						$_POST['j17'][$i],
						$_POST['j18'][$i],
						$_POST['j19'][$i],
			...
			$item_idCount = $_POST["i"];
				...
				for($i=0; $i<$item_idCount; $i++) {
					...
					// add parameterized query
					$result = sqlInsert(
						'INSERT INTO epenilaian_ticer (e_pid, e_tid, e_j17, e_j18, e_j19, e_date) VALUES (?, ?, ?, ?, ?, ?)', 
						array(
							$_POST['pid'], 
							$_POST['tid'][$i], 
							$_POST['j17'][$i],
		...
		// add parameterized query
		$result = sqlInsert(
			'INSERT INTO epenilaian (e_pid, e_j1, e_j2, e_j3, e_j4, e_j5, e_j6, e_j7, e_j8, e_j9, e_j10, e_j11, e_j12, e_j13, e_j14, e_j15, e_j16, e_j20, e_j21, e_j22, e_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
			array(
				$_POST['pid'], 
				$_POST['j1'], 
				$_POST['j2'], 
	...
	if($_POST['j1'] > 0 && $_POST['j2'] > 0 && $_POST['j3'] > 0 && $_POST['j4'] > 0 && $_POST['j5'] > 0 && $_POST['j6'] > 0 && $_POST['j7'] > 0 && $_POST['j8'] > 0 && $_POST['j9'] > 0 && $_POST['j10'] > 0 && $_POST['j11'] > 0 && $_POST['j12'] > 0 && $_POST['j13'] > 0 && $_POST['j14'] > 0 && $_POST['j15'] > 0 && $_POST['j16'] > 0 && $_POST['j20'] > 0 && $_POST['j21'] > 0 && $_POST['j22'] != "" && $_POST['j17'] > 0 && $_POST['j18'] > 0 && $_POST['j19'] > 0){
		...
		// add parameterized query
		$result = sqlUpdate(
			'UPDATE pilih SET p_epstatus = ? WHERE p_id = ?', 
			array('12', $_POST['pid'])
		);
