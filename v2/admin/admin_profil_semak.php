...
if(isset($_SESSION['terai']) && $_SESSION['terai'] == "admprofil") {
	...
	if($rowCount['total'] == 1) {
		...
		// add parameterized query
		$result = sqlInsert(
			'INSERT INTO admin (a_pwd, a_idnum, a_nama, a_tel, a_emel, a_level, a_status) VALUES (?, ?, ?, ?, ?, ?)', 
			array(
				$pwd,
				$_SESSION['idnum'],
				$_SESSION['nama'],
	...
	// add parameterized query
	$rowCount = sqlSelect(
		'SELECT count(*) AS total FROM admin WHERE a_idnum LIKE ? ORDER BY a_id ASC', 
		array($_SESSION['idnum'])
	);
	$result = sqlSelect(
		'SELECT * FROM admin WHERE a_idnum LIKE ? ORDER BY a_id ASC', 
		array($_SESSION['idnum'])
	);
