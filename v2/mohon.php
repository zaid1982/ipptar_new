...
<div id="content">
  ...
  <form action="" method="GET">
    <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
      <tr>
        <td style="font-weight:bold">MOHON KURSUS</td>
      </tr>
      <tr>
        <td>
          ...
          $querysum = "SELECT COUNT(b_id) AS total
          ...
          // add parameterized query
          /* $rowsum  = sqlSelect(
            'SELECT COUNT(b_id) AS total FROM kluster a, kursus b WHERE a.b_id = b.k_bid AND (b.k_sdate > ? OR b.k_wlkin = ?) AND b.k_sdate > ? AND b.k_status = ? ORDER BY a.b_id ASC', 
            array($now, '1', '2020-12-31 00:00:00', '7')
          ); */
          ?>
  ...
  <table width="100%" align="center" cellpadding="3" cellspacing="1" bgcolor="#FFFFFF">
    <tr>
      <td width="5%" align="center"><strong>No.</strong>
      </td>
      <td width="50%" nowrap><strong>Kursus</strong>
      </td>
      <td width="15%" align="center" nowrap><strong>Kluster</strong>
      </td>
      <td width="10%" align="center" nowrap><strong>Mula Kursus</strong>
      </td>
      <td width="10%" align="center" nowrap><strong>Tamat Kursus</strong>
      </td>
      <td width="10%" align="center" nowrap><strong>Status</strong>
      </td>
    </tr>
    <?php
    ...
    // add parameterized query
    /* $result  = sqlSelect(
      'SELECT * FROM kluster a, kursus b WHERE a.b_id = b.k_bid AND (b.k_sdate > ? OR b.k_wlkin = ?) AND b.k_sdate > ? AND b.k_status = ? ORDER BY b.k_sdate ASC, a.b_id ASC LIMIT ?, ?', 
      array($now, '1', '2020-12-31 00:00:00', '7', $offset, $list_perpage), 
      true
    ); */
    ...
    ?>
      <tr>
        <td <?php print $tdbg; ?> align='center' valign='middle' nowrap>
          <font class='txt_caption'><?php print $i + ($page - 1) * 20 ?></font>
        </td>
        <?php
        if ($row['k_status'] == '14') {
        ?>
        ...
        ?>
        <td <?php print $tdbg; ?> nowrap>
          <font class='txt_caption'><?php print $row["b_name"]; ?></font>
        </td>
        <td <?php print $tdbg; ?> align='center'>
          <font class='txt_caption'><?php print date("d-m-Y", strtotime($row["k_sdate"])); ?></font>
        </td>
        <td <?php print $tdbg; ?> align='center'>
          <font class='txt_caption'><?php print date("d-m-Y", strtotime($row["k_edate"])); ?></font>
        </td>
        <td <?php print $tdbg; ?> align='center'>
          <font class='txt_caption'><?php print $status; ?></font>
        </td>
      </tr>
    <?php
      $i++;
    ...
    <tr>
      <td colspan="6" align="center">
        ...
        // add parameterized query
        /* $row_count  = sqlSelect('SELECT COUNT(b_id) AS total FROM kluster a, kursus b WHERE a.b_id = b.k_bid AND (b.k_sdate > ? OR b.k_wlkin = ?) AND b.k_sdate > ? AND b.k_status = ? ORDER BY a.b_id ASC', array($now, '1', '2020-12-31 00:00:00', '7')); */

        @$page = abs(intval($_GET['page']));
        if (empty($page) || $page > ceil($row_count[0] / $limit)) {
          $page = 1;
          $start = 0;
        } else {
          $start = ($page - 1) * $limit;
        ...
        print paging($limit, $page, $row_count[0], "mohon.php?show=files&search=$search&kluster=$kluster&bulan=$bulan");
