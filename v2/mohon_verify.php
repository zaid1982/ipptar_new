...
if(isset($_GET['pid']) && isset($_GET['kid']) && isset($_GET['s'])) {
	...
	$row = sqlSelect(
		'SELECT p_id FROM pilih WHERE p_id LIKE ? AND p_kid LIKE ? ORDER BY p_id ASC', 
		array($pid, $kid)
	);
	$rowCount = sqlSelect(
		'SELECT count(p_id) AS total FROM pilih WHERE p_id LIKE ? AND p_kid LIKE ? ORDER BY p_id ASC', 
		array($pid, $kid)
	);
  ...
  if($rowCount['total'] != "0"){
		...
		// add parameterized query
		$result = sqlUpdate(
			'UPDATE pilih SET p_status = ? WHERE p_id = ?', 
			array($status, $row['p_id'])
		);
