...
if(isset($_SESSION['terai']) && $_SESSION['terai'] == "profil"){
	...
	$rowCount	= sqlSelect(
		'SELECT count(u_id) AS total FROM user WHERE u_id = ? ORDER BY u_id ASC', 
		array($_SESSION['UsrID'])
	);
	$row = sqlSelect('SELECT * FROM user WHERE u_id = ? ORDER BY u_id ASC', array($_SESSION['UsrID']));
	
	// if($numrows == "1"){
	if ($rowCount['total'] == 1) {

		/* $sql = "UPDATE user SET u_nama = '$_SESSION[nama]',
		 u_jantina = '$_SESSION[jantina]', u_dob = '$tkhlahir', u_tel = '$_SESSION[tel]', u_jwt = '$_SESSION[jawatan]', u_jwttingkat = '$_SESSION[peringkat]', u_jwtklas = '$_SESSION[klasifikasi]', u_jwtgred = '$_SESSION[gred]', u_jwttaraf = '$_SESSION[taraf]', u_jkhidmat = '$_SESSION[khidmat]', u_tkhlantik = '$tkhlantik', u_emel = '$_SESSION[emel]', u_knama = '$_SESSION[ketua]', u_kjwt = '$_SESSION[ketuajwt]', u_kemel = '$_SESSION[ketuaemel]', u_alamatkjab = '$_SESSION[alamatkjab]', u_jab = '$_SESSION[jab]', u_jabaddr1 = '$_SESSION[jabaddr1]', u_jabaddr2 = '$_SESSION[jabaddr2]', u_jabpkod = '$_SESSION[jabpkod]', u_jabbandar = '$_SESSION[jabbandar]', u_jabnegeri = '$_SESSION[jabnegeri]', u_jabtel = '$_SESSION[jabtel]', u_jabfax = '$_SESSION[jabfax]' WHERE u_idnum = '$_SESSION[idnum]'";
		$result = mysql_query($sql) or die(mysql_error()); */
		
		// add parameterized query
		$sql = sqlUpdate(
			'UPDATE user SET u_nama = ?, u_jantina = ?, u_dob = ?, u_tel = ?, u_jwt = ?, u_jwttingkat = ?, u_jwtklas = ?, u_jwtgred = ?, u_jwttaraf = ?, u_jkhidmat = ?, u_tkhlantik = ?, u_emel = ?, u_knama = ?, u_kjwt = ?, u_kemel = ?, u_alamatkjab = ?, u_jab = ?, u_jabaddr1 = ?, u_jabaddr2 = ?, u_jabpkod = ?, u_jabbandar = ?, u_jabnegeri = ?, u_jabtel = ?, u_jabfax = ? WHERE u_idnum = ?', 
			array(
				$_SESSION['nama'], 
				$_SESSION['jantina'], 
				$tkhlahir, 
